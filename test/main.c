...
int je_na_kruznici(int r, int s, int max_r, int max_s) {
    double stred_r = max_r, stred_s = 1, polomer = 8;

    double vzdalenost = (s - stred_s) * (s - stred_s) + (r - stred_r) * (r - stred_r);
    double dolni_mez = (polomer - 0.5) * (polomer - 0.5);
    double horni_mez = (polomer + 0.5) * (polomer + 0.5);

    return dolni_mez < vzdalenost && vzdalenost < horni_mez;
}

// vypise obrazek
int vypis_obrazek(int r, int s, int max_r, int max_s) {
     for (r = 1; r <= max_r; r++) {//Prvni for cyklus vzdy reprezentuje postup po radcich
        for (s = 1; s <= max_s; s++) {//Druchy for cyklus vzdy reprezentuje postup po sloupcich
             ...
             * Podminka se pta zda soucet je cisla aktualniho radku a sloupce roven celkovemu poctu radku + 1
             ...
             * prakticky to znamena ze znak - vypisujem na ve vsech sloupcich radku 3,4 a 5.
             ...
             *     |X    |
             */
            } else if (je_na_kruznici(r,s,max_r,max_s)) {
                 printf("0");//vypise -
